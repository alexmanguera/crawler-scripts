...
====================
Job Portal Crawler - ejobs.ro
Current Version: 1.0.0
====================

Change Log:
ver 1.0.0
- Script that crawls ejobs.ro Jobs section that captures specific fields.
...
**************/
								...
								)
	{
		...
		$sql = "INSERT INTO job_details (
									...
									ON DUPLICATE KEY UPDATE 
										...
										foreign_language = '$output_foreignLanguage'
									";
	...
	$website_root = 'https://www.ejobs.ro/locuri-de-munca';
	$website_page_contents = getURLContents($website_root);
	
	$array_captured_link_a = array();
	
	$get_start = '<li class="last"> <a href="https://www.ejobs.ro/locuri-de-munca';
	$get_end = '"';
	...
	if(strpos($website_page_contents, $get_start) > strlen($get_start))
	{
		...
		$total_jobs = str_replace('page', "", $total_jobs);
	...
	{	
		...
		$get_start = '<ul id="job-app-list" class="clearfix">';
		$get_end = '<!-- #job-app-list -->';
		...
		{ 
			...
			while(strpos($captured, $get_start) > strlen($get_start))
			{
				...
				$captured_link = $captured_link_raw;
	...
	function scrape_detailed_page($pdo, $scrape_page, $job_portal, $counter)
	{
		...
		// -------------------------------------
		// Job Industry
		$get_start = 'data-plural="industrii" data-size="5">';
		$get_end = '</ul>';
		...
		{ 
			...
			$job_industry = str_replace('<li class="block-list-item">', "", $job_industry);
			$job_industry = str_replace('</a></li>', ",", $job_industry);
			$job_industry = substr($job_industry, strpos($job_industry, '">') + 2, strpos($job_industry, ","));
			$job_industry = str_replace(',', "", $job_industry);
			$job_industry = trim($job_industry);
		}
		else
		{
			// select the Department if Industry is not available.
			$get_start = 'data-plural="departamente" data-size="5">';
			$get_end = '</ul>';
		...
		// -------------------------------------
		// Job Title
		$get_start = '<h1 class="jobad-title" itemprop="title">';
		$get_end = '</h1>';
		...
		// -------------------------------------
		// Job Description
		$get_start = '<section class="jobad-content grid_17 suffix_1 ">';
		$get_end = ' <div class="jobad-btn-apply-container">';
		...
		// -------------------------------------
		// Employment Type
		$get_start = '<ul class="block-list" itemprop="employmentType">';
		$get_end = '</ul>';
		...
		{ 
			...
			$output_employmentType = str_replace('<li class="block-list-item">', "", $output_employmentType);
			$output_employmentType = str_replace('</a></li>', ",", $output_employmentType);
			$output_employmentType = substr($output_employmentType, strpos($output_employmentType, '">') + 2, strpos($output_employmentType, ","));
			$output_employmentType = str_replace(',', "", $output_employmentType);
		...
		$get_start = '<span itemprop="baseSalary">';
		$get_end = '</span>';
		...
		// -------------------------------------
		// Location
		$get_start = 'data-singular="oras" data-plural="orase" data-size="5">';
		$get_end = '</ul>';
		...
		{ 
			...
			$output_location = str_replace('<li class="block-list-item">', "", $output_location);
			$output_location = str_replace('</a></li>', ",", $output_location);
			$output_location = substr($output_location, strpos($output_location, '">') + 2, strpos($output_location, ","));
		...
		// -------------------------------------
		// Company Name
		$get_start = '<div class="company-name" itemprop="name">';
		$get_end = '</div>';
		...
		// -------------------------------------
		// Foreign Language
		$get_start = 'data-plural="limbi straine" data-size="5">';
		$get_end = '</ul>';
		...
		{ 
			...
			$output_foreignLanguage = str_replace('<li class="block-list-item">', "", $output_foreignLanguage);
			$output_foreignLanguage = str_replace('</a></li>', ",", $output_foreignLanguage);
			$output_foreignLanguage = substr($output_foreignLanguage, strpos($output_foreignLanguage, '">') + 2, strpos($output_foreignLanguage, ","));
			$output_foreignLanguage = str_replace(',', "", $output_foreignLanguage);
