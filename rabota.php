...
====================
Job Portal Crawler - rabota.md
Current Version: 1.1.0
====================

Change Log:
ver 1.0.0
- Script that crawls rabota.md Jobs section that captures specific fields.
ver 1.1.0
- Add limit on the number of pages to crawl per Job Category/Industry.
...
**************/
	...
	$website_root = 'http://www.rabota.md/';
	$website_page_contents = getURLContents($website_root);

	$array_captured_link_a = array();

	$get_start = '<div class="main-content" style="width: 100%; float: none; overflow: hidden; margin-left:10px;">';
	$get_end = '<div id="tab_4" class="indexTab hidden">';
	...
	{ 
		...
		$get_start = '<a style="font-weight: bold;" href="http://www.rabota.md/vacancies/vacancyCategory.php?id=';
		$get_end = '</a>';
		...
		while(strpos($captured, $get_start) > strlen($get_start))
		{
			...
			$captured_link = str_replace('<a style="font-weight: bold;" href="', '', $get_start).$captured_link_raw;
			$captured_link = str_replace('">', '', $captured_link);
			$captured_link = str_replace($captured_industry, '', $captured_link);
	...
	foreach($array_captured_link_a as $key => $value)
	{
		...
		$get_start = '<td valign="top" width="705">';
		$get_end = '<p align="right">';
		...
		if(strpos($website_page_contents, $get_start) > strlen($get_start))
		{
			...
			while($valid == true)
			{
				...
				$get_start = '<a target="_blank" class="link " href="';
				$get_end = '"';
				...
				if(strpos($website_page_contents, $get_start) > strlen($get_start))
				{
					...
					while(strpos($captured, $get_start) > strlen($get_start))
					{
						...
						$captured_link = 'http://www.rabota.md/vacancies/'.$captured_link;
	...
	function scrape_detailed_page($pdo, $job_industry, $scrape_page, $job_portal, $counter)
	{
		...
		// -------------------------------------
		// Email
		$get_start = '<a style="font-size: 18px; font-weight: normal;" href="mailto:';
		$get_end = '">';
		...
		{ 
			...
			$output_email = trim(substr($website_page_contents, $select_pos_start, $select_length));
		}
		else
		{
			$output_email = '';
		}
		$extractedEmail = array($output_email);
		$output_email = (!empty($extractedEmail)) ? json_encode($extractedEmail) : NULL;
		
		
		// Phone
		$get_start = '<div id="vacContactInfoNew" style="padding-top:15px;">';
		$get_end = '</div>';
		...
		{ 
			...
			$output_phone = trim(substr($website_page_contents, $select_pos_start, $select_length));
		}
		else
		{
			$output_phone = '';
		}
		$extractedPhone = extract_phone($output_phone);
		$output_phone = (!empty($extractedPhone)) ? json_encode($extractedPhone) : NULL;
		
		
		// -------------------------------------
		// Career Level
		$get_start = 'itemprop="educationRequirements">';
		$get_end = '</ul>';
		...
		{ 
			...
			$output_careerLevel = str_replace('<li class="block-list-item">', "", $output_careerLevel);
			$output_careerLevel = str_replace('</a></li>', ",", $output_careerLevel);
			$output_careerLevel = substr($output_careerLevel, strpos($output_careerLevel, '">') + 2, strpos($output_careerLevel, ","));
			$output_careerLevel = str_replace(',', "", $output_careerLevel);
		...
		// -------------------------------------
		// Employment Type
		$get_start = '<div class="item graphIco">';
		$get_end = '</div>';
		...
		{ 
			...
			$output_employmentType = str_replace('<span>График работы:</span>', "", $output_employmentType);
			$output_employmentType = str_replace('<span class="value">', "", $output_employmentType);
			$output_employmentType = str_replace('</span>', "", $output_employmentType);
		...
		$get_start = '<div class="item experienceIco">';
		$get_end = '</div>';
		...
		{ 
			...
			$output_MinWorkExperience = str_replace('<span>Зарплата:</span>', "", $output_MinWorkExperience);
			$output_MinWorkExperience = str_replace('<span class="value">', "", $output_MinWorkExperience);
		...
		$get_start = '<div class="item educationIco">';
		$get_end = '</div>';
		...
		{ 
			...
			$output_MinEducationLevel = str_replace('<span>Образование:</span>', "", $output_MinEducationLevel);
			$output_MinEducationLevel = str_replace('<span class="value">', "", $output_MinEducationLevel);
		...
		// -------------------------------------
		// Location
		$get_start = '<div class="item cityIco">';
		$get_end = '</div>';
		...
		{ 
			...
			$output_location = str_replace('<span>Город:</span>', "", $output_location);
			$output_location = str_replace('<span class="value">', "", $output_location);
		...
		{ 
			...
			if(strpos($output_companyName, '<a target=') !== false)
			{
		...
		// -------------------------------------
		// Company Logo
		$get_start = '<img class="elogo__img" src="https://';
		$get_end = '"';
		...
		{ 
			...
			$output_companyLogo = 'https://'.$output_companyLogo;
		}
		else
		{
			$output_companyLogo = '';
		...
		// -------------------------------------
		// Job Date Posted
		$get_start = 'xxxxxxxxxxxx';
		$get_end = 'xxxxxxxxxxxx';
