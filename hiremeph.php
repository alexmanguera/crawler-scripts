...
====================
Job Portal Crawler - hireme.ph
Current Version: 1.0.1
====================

Change Log:
ver 1.0.0
- Script that crawls hireme.ph Jobs section that captures specific fields.
...
**************/
	...
	{ 
		...
		while(strpos($captured, $get_start) > strlen($get_start))
		{
			...
			$captured_link = substr($captured_link_raw, strpos($captured_link_raw, '<a href="') + 9, strlen($captured_link_raw));
			$captured_link = str_replace('">'.$captured_industry, "", $captured_link);
			
			$captured_count = substr($captured_link_raw, 0, strpos($captured_link_raw, '</strong>'));
			
			$array_captured_link_a[] = array($captured_industry, $captured_link, $captured_count);
	...
	foreach($array_captured_link_a as $key => $value)
	{
		$raw_url = $value[1];
		$total_jobs = $value[2];
		$last_page = ceil($total_jobs / 50);
		
		$website_page_contents = getURLContents($raw_url);
		
		$get_start = '<!-- #sort-by-type -->';
		$get_end = '<!-- #job-listings -->';
		
		if(strpos($website_page_contents, $get_start) > strlen($get_start))
		{
			for($i = 0; $i <= $last_page; $i++)
			{
				$website_page_contents = getURLContents($raw_url.'?p='.$i);
				
				$get_start = '<a href="http://www.hireme.ph/job/';
				$get_end = '"';
				...
				if(strpos($website_page_contents, $get_start) > strlen($get_start))
				{
					...
					while(strpos($captured, $get_start) > strlen($get_start))
					{
						...
						$captured_link = 'http://www.hireme.ph/job/'.$captured_link;
	...
	function scrape_detailed_page($pdo, $job_industry, $scrape_page, $job_portal, $counter)
	{
		...
		// -------------------------------------
		// Job Title
		$get_start = 'alt="Email Us" align="right"/>';
		$get_end = '</h2>';
		...
		{ 
			...
			$output_jobTitle = preg_replace('/\s+/', '', $output_jobTitle);
		...
		// -------------------------------------
		// Job Description
		$get_start = '<div id="job-description">';
		$get_end = '</div>';
		...
		{ 
			...
			$output_employmentType = substr($output_employmentType, strpos($output_employmentType, 'alt="') + 5, strlen($output_employmentType));
		...
		// -------------------------------------
		// Location (City)
		$get_start = '<span class="fading">in</span>';
		$get_end = '</strong>';
		...
		// -------------------------------------
		// Company Name
		$get_start = '<span class="fading">at</span>';
		$get_end = '</strong>';
