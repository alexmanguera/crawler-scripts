...
====================
Job Portal Crawler - work.ua
Current Version: 1.0.0
====================

Change Log:
ver 1.0.0
- Script that crawls work.ua Jobs section that captures specific fields.
...
**************/
	...
	$website_root = 'https://www.work.ua/jobs/by-category/';
	$website_page_contents = getURLContents($website_root);
	
	$array_captured_link_a = array();

	$get_start = '<div id="js-ajax-container" class="row">';
	$get_end = '<ul class="list-inline menu-my-img">';
	...
	{ 
		...
		while(strpos($captured, $get_start) > strlen($get_start))
		{
			...
			$captured_link = str_replace('<a href="', '', $captured_link_raw);
			$captured_link = str_replace('">', '', $captured_link);
			$captured_link = str_replace($captured_industry, '', $captured_link);
			
			$array_captured_link_a[] = array($captured_industry, 'https://www.work.ua'.$captured_link);
	...
	foreach($array_captured_link_a as $key => $value)
	{
		...
		$get_start = '<h1 class="add-top text-basic text-muted">';
		$get_end = '&nbsp;';
		...
		if(strpos($website_page_contents, $get_start) > strlen($get_start))
		{
			...
			$last_page = ceil($total_jobs / 14);
		
			//print_r($total_jobs."\r");
			//continue;
			...
			for($i = 0; $i <= $last_page; $i++)
			{
				...
				$get_start = '<div class="col-md-8 col-left">';
				$get_end = '<ul class="pagination hidden-xs">';
				
				if(strpos($website_page_contents, $get_start) > strlen($get_start))
				{
					$get_start = '<h2><a href="/jobs/';
					$get_end = '"';
					...
					if(strpos($website_page_contents, $get_start) > strlen($get_start))
					{
						...
						while(strpos($captured, $get_start) > strlen($get_start))
						{
							...
							$captured_link = 'https://www.work.ua/jobs/'.$captured_link;
	...
	function scrape_detailed_page($pdo, $job_industry, $scrape_page, $job_portal, $counter)
	{
		...
		// -------------------------------------
		// Job Title
		$get_start = '<h1 class="cut-top wordwrap" id="h1-name">';
		$get_end = '</h1>';
		...
		// -------------------------------------
		// Job Description
		$get_start = '<span>Описание вакансии</span></h2>';
		$get_end = '<a href="/noscript/" onclick="return false;"';
		...
		{ 
			...
			$output_phone = str_replace('<dd>', "", $output_phone);
		}
		else
		{
			$output_phone = '';
		}
		$extractedPhone = array($output_phone);
		...
		{ 
			...
			$output_monthlySalaryRange = str_replace('</b>', "", $output_monthlySalaryRange);
		...
		{ 
			...
			$output_companyName = substr($output_companyName, strpos($output_companyName, '<b>') + 3, strlen($output_companyName));
		}
		else
		{
			$output_companyName = '';
		...
		// -------------------------------------
		// Company Logo
		$get_start = '<img src="//i.work.ua/employer_design/';
		$get_end = '"';
		...
		{ 
			...
			$output_companyLogo = 'https://i.work.ua/employer_design/'.$output_companyLogo;
		}
		else
		{
			$output_companyLogo = '';
