...
====================
Job Portal Crawler - dubai-bb.com
Current Version: 1.0.0
====================

Change Log:
ver 1.0.0
- Script that crawls dubai-bb.com Jobs section that captures specific fields.
...
**************/
	...
	$website_root = 'http://dubai-bb.com/dubai-classifieds-41.html';
	...
	{ 
		...
		while(strpos($captured, $get_start) > strlen($get_start))
		{
			...
			$captured_link = "http://dubai-bb.com/".$captured_link;
	...
	foreach($array_captured_link_a as $key => $value)
	{
		...
		{
			$raw_url = str_replace(".html", "", $raw_url);
			$website_page_contents = getURLContents($raw_url.'-'.$i.".html");
			
			$get_start = 'There are no ads in this category';
			...
			{
				$get_start = 'http://dubai-bb.com/images/dubai_classifieds/dubai_jobs.png"';
				$get_end = '<div class="advertHead">';
				...
				if(strpos($website_page_contents, $get_start) > strlen($get_start))
				{
					...
					while(strpos($captured, $get_start) > strlen($get_start))
					{
						...
						$captured_link = "http://dubai-bb.com/dubai-advert".$captured_link;
	...
	function scrape_detailed_page($pdo, $job_industry, $scrape_page, $job_portal, $counter)
	{
		...
		// -------------------------------------
		// Job Title
		$get_start = '<div class="adContent"><h1>';
		$get_end = '</h1>';
		...
		// -------------------------------------
		// Job Description
		$get_start = '<span class="postbody">';
		$get_end = '<div class="bellowTextPub">';
		...
		$get_start = 'Minimum Experience</div>';
		$get_end = '</div>';
		...
		{ 
			...
			$output_MinWorkExperience = substr($output_MinWorkExperience, strpos($output_MinWorkExperience, '">') + 2, strlen($output_MinWorkExperience));
