...
====================
Job Portal Crawler - tanitjobs.com
Current Version: 1.0.0
====================

Change Log:
ver 1.0.0
- Script that crawls tanitjobs.com Jobs section that captures specific fields.
...
**************/
	...
	$website_root = 'https://www.tanitjobs.com/jobs/';
	$website_page_contents = getURLContents($website_root);
	
	$array_captured_link_a = array();
	
	$get_start = '<h1 class="search-results__title col-sm-offset-3 col-xs-offset-0">';
	$get_end = 'annonces trouvées';
	...
	while($i < $total_jobs)
	{
		$x = "4".rand(00001,99999);
		$current_page = 'https://www.tanitjobs.com/job/'.$x;
		
		$website_page_contents = getURLContents($current_page);
		...
		if(strpos($website_page_contents, $get_start) > strlen($get_start))
		{
			...
			// --------------------------------------------------------
			if(!checkIfSkip($pdo, $current_page, $job_portal)) {
				
				//--------------------------------------
				if(in_array($current_page, $jobs_id_temp_array))
				{						
					//continue;
					echo "YES";
				}
				...
				{
					//print_r($current_page."\r");
					
					$jobs_id_temp_array[] = $current_page;
					// -------------------
					// scrape the detailed job page (last page)
					scrape_detailed_page($pdo, $current_page, $job_portal, $counter);
					// -------------------		
				}
				//--------------------------------------
				...
				//print_r('SKIPPING - ' .$counter.' - '.$current_page."\r");
				continue;
			}
			//break;
			
		}else{
			//print_r("SKIPPING ".$current_page."\r");
			continue;
		}
