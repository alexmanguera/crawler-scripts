...
====================
Job Portal Crawler - careersingulf.com
Current Version: 1.0.1
====================

Change Log:
ver 1.0.0
- Script that crawls careersingulf.com Jobs section that captures specific fields.
ver 1.0.1
- Fixed the placement of checkIfSkip() function.
...
**************/
	...
	$valid_next_page = true;
	
	while($valid_next_page == true)
	{
		$i++;
		
		$website_root = 'http://www.careersingulf.com/page'.$i.'?action=search';
		$website_page_contents = getURLContents($website_root);
		
		$array_captured_link_a = array();

		$get_start = 'onmouseover="old_bg=this.getAttribute(';

		if(strpos($website_page_contents, $get_start) > strlen($get_start))
		{
			$get_start = '<table id="joblist" align="center" border="0" cellSpacing="1" cellPadding="5" class="list">';
			$get_end = '<p class="nav_page_links">';
			...
			if(strpos($website_page_contents, $get_start) > strlen($get_start))
			{
				...
				$get_start = '<a href="http://www.careersingulf.com/job/';
				$get_end = '"';
				...
				while(strpos($captured, $get_start) > strlen($get_start))
				{
					...
					$captured_link = 'http://www.careersingulf.com/job/'.$captured_link;
	...
	function scrape_detailed_page($pdo, $scrape_page, $job_portal, $counter)
	{
		...
		// -------------------------------------
		// Job Description
		$get_start = '<div style="margin: 5px 3px 18px 0px; clear:both;">';
		$get_end = '</div>';
		...
		{ 
			...
			$output_email_option = trim(substr($website_page_contents, $select_pos_start, $select_length));
			$output_email_option = str_replace('<td class="dynamic_form_value"  >', "", $output_email_option);
			$output_email_option = str_replace('<img src="http://www.careersingulf.com/include/themes/nasthon1007/images/at.gif" width="13" height="9" border="0" ALT="(at)">', "@", $output_email_option);
			$output_email_option = addslashes(trim(strip_tags($output_email_option)));
		}
		else
		{
			$output_email_option = '';
		...
		{ 
			...
			$output_telephone_option = trim(substr($website_page_contents, $select_pos_start, $select_length));
			$output_telephone_option = str_replace('<td class="dynamic_form_value"  >', "", $output_telephone_option);
			$output_telephone_option = addslashes(trim(strip_tags($output_telephone_option)));
		}
		else
		{
			$output_telephone_option = '';
		}
		
		// -------------------------------------
		// Email
		$extractedEmail = extract_email_address($output_jobDescription.' '.$output_email_option);
		$extractedPhone = extract_phone($output_jobDescription.' '.$output_telephone_option);
		...
		{ 
			...
			$output_employmentType = substr($output_employmentType, strpos($output_employmentType, '">') + 2, strlen($output_employmentType));
			$output_employmentType = addslashes(trim(strip_tags($output_employmentType)));
			$output_employmentType = str_replace('amp;', "", $output_employmentType);
		}
		else
		{
			$output_employmentType = '';
		...
		{ 
			...
			$output_monthlySalaryRange = str_replace('<td class="dynamic_form_value"  >', "", $output_monthlySalaryRange);
		...
		{ 
			...
			$output_location = substr($output_location, strpos($output_location, '">') + 2, strlen($output_location));
		...
		{ 
			...
			$output_companyName = str_replace('<td class="dynamic_form_value"  >', "", $output_companyName);
			$output_companyName = addslashes(trim(strip_tags($output_companyName)));
			$output_companyName = str_replace('amp;', "", $output_companyName);
		}
		else
		{
			$output_companyName = '';
		...
		{ 
			...
			$output_industry = substr($output_industry, strpos($output_industry, 'jobs.html">') + 11, strlen($output_industry));
			$output_industry = addslashes(trim(strip_tags($output_industry)));
			$output_industry = str_replace('amp;', "", $output_industry);
		}
		else
		{
			$output_industry = '';
		...
		{ 
			...
			$output_jobFunction = trim(substr($website_page_contents, $select_pos_start, $select_length));
			$output_jobFunction = substr($output_jobFunction, strpos($output_jobFunction, 'jobs.html">') + 11, strlen($output_jobFunction));
			$output_jobFunction = addslashes(trim(strip_tags($output_jobFunction)));
			$output_jobFunction = str_replace('amp;', "", $output_jobFunction);
		}
		else
		{
			$output_jobFunction = '';
		...
		if(strpos($website_page_contents, $get_start) > strlen($get_start))
		{
			...
			{ 
				...
				$output_nationality = substr($output_nationality, strpos($output_nationality, '">') + 2, strlen($output_nationality));
		...
		else
		{
			...
			{ 
				...
				$output_nationality = trim(substr($website_page_contents, $select_pos_start, $select_length));
				$output_nationality = str_replace('<td class="dynamic_form_value"  >', "", $output_nationality);
				$output_nationality = addslashes(trim(strip_tags($output_nationality)));
				$output_nationality = str_replace('amp;', "", $output_nationality);
			}
			else
			{
				$output_nationality = '';
		...
		{ 
			...
			$output_gender = str_replace('<td class="dynamic_form_value"  >', "", $output_gender);
		...
		insertDBJobDetails(
			...
			print_r('PROCESSING - ' .$counter.' - '.$scrape_page."\r");
